...
typedef enum NELPMovieLoadState {
    NELPMovieLoadStatePlayable       = 1 << 0, //!< 在该状态下，播放器初始化完成，可以播放，或者在网络不好的情况下缓冲完成，可以播放
    NELPMovieLoadStatePlaythroughOK  = 1 << 1, //!< 在该状态下，shouldAutoplay 设置成YES，播放器初始化完成后会自动播放；或者在网络不好的情况下缓冲完成，可以播放
 ...
 *
 *  NEPMovieLoadStatePlayable       = 1 << 0, // 在该状态下，播放器初始化视频文件完成，可以播放，或者在网络不好的情况下缓冲完成，可以播放 \\\n
 *  NEPMovieLoadStatePlaythroughOK  = 1 << 1, // 在该状态下，shouldAutoplay 设置成YES，播放器初始化视频文件完成后会自动播放；或者在网络不好的情况下缓冲完成，可以播放 \\\n
 *  NEPMovieLoadStateStalled        = 1 << 2, // 在播放过程中网络不好需要缓冲数据的时候播放会自动暂停 \\n
 *
 *  加载状态的变换如下：\\\n
 *  调用prepareToPlay方法，当视频文件初始化完成后，播放器会收到NELivePlayerLoadStateChangedNotification通知，此时的加载状态为 NELPMovieLoadStatePlayable | NELPMovieLoadStatePlaythroughOK \\\n
 *  当播放器需要缓冲的时候，缓冲开始时，播放会暂停，此时播放器会收到NELivePlayerLoadStateChangedNotification通知，此时的加载状态为 NEPMovieLoadStateStalled \\\n
 *  当缓冲结束时，播放会继续，此时播放器会收到NELivePlayerLoadStateChangedNotification通知，此时的加载状态为 NELPMovieLoadStatePlayable | NELPMovieLoadStatePlaythroughOK
 *
 *	@return	当前加载状态
 */
